...
# Splitting the dataset into the Training set and Test set [§12 Lect91: "Logistic Regression - R pt1"]
...
# Feature Scaling [§12 Lect91: "Logistic Regression - R pt1"]
# ...Video had `[, 1:2]` not `[-3]`, but in R the latter means remove the 3rd col (leaving 1&2)
training_set[-3] = scale(training_set[-3])
test_set[-3] = scale(test_set[-3])

# Fitting Logistic Regression to the Training set [§12 Lect92: "Logistic Regression - R pt2"]
classifier = glm(formula = Purchased ~ .,
                 family = binomial,
                 data = training_set)

# Predicting the Test set results [§12 Lect93: "Logistic Regression - R pt3"]
prob_pred = predict(classifier, type = 'response', newdata = test_set[-3])
y_pred = ifelse(prob_pred > 0.5, 1, 0)

# Making the Confusion Matrix [§12 Lect94: "Logistic Regression - R pt4"]
cm = table(test_set[, 3], y_pred > 0.5)

# Visualising the Training set results [§12 Lect95: "Logistic Regression - R pt5"]
...
# Visualising the Test set results [§12 Lect95: "Logistic Regression - R pt5"]
library(ElemStatLearn)
set = test_set
X1 = seq(min(set[, 1]) - 1, max(set[, 1]) + 1, by = 0.01)
X2 = seq(min(set[, 2]) - 1, max(set[, 2]) + 1, by = 0.01)
...
# ...[§12 Lect95: "Logistic Regression - R pt5" Q&A]
